...
def pattern(zome):
    ...
    Y = np.array(zome.pixels)[:,1] # map to image x
    Z = np.array(zome.pixels)[:,2] # map to image y
    while True: 
        for f in os.listdir(img_dir):
            ...
            if not (len(img.shape) == 3 and img.shape[2] == 3):
                sys.stderr.write(f"image file does not have the right shape {img.shape}. File: {f}\n")
                continue
            img_dim = img.shape
            ...
            for _ in range(total_frames):
                ...
                zoomed_img = img[zoomed_x:zoomed_x + zoomed_width, zoomed_y:zoomed_y + zoomed_height]
                zoomed_img_dim = zoomed_img.shape 

                rotate_z = frame_id/zome.fps * max_y
                ind_X = ((Y + max_y + rotate_z) % (2*max_y))/(2*max_y) * zoomed_img_dim[0]
                ind_Y = Z/max_z * zoomed_img_dim[1]
